...
* 1 1) 에서 발급된 authorization code 와 API client app 의 secret 정보를 파라미터로 전송함.
* 2 accessType을 offline 으로 설정하여, resource owner 가 client app을 사용중이지 않은 시점에도 access token 갱신이 가능하도록 함.
* : access token 갱신 시에 사용할 refresh token 을 발급받기 위함.
* 3 발급받은 token information 을 바탕으로 Credential 을 생성하고, Credential store(DataStore) 를 이용해 저장함.
...
//	private static final Collection<String> SCOPES = Arrays.asList(GmailScopes.MAIL_GOOGLE_COM);
//https://developers.google.com/identity/protocols/oauth2/scopes
	private static final Collection<String> SCOPES = Arrays.asList(GmailScopes.GMAIL_READONLY,GmailScopes.MAIL_GOOGLE_COM,
	...
	public void init() throws GeneralSecurityException, IOException {
		...
		logger.info("clientSecretFile 값 추출 : {}  ", clientSecretFile.getInputStream());
		...
		authorizationCodeFlowBuilder = new GoogleAuthorizationCodeFlow.Builder(HTTP_TRANSPORT, JSON_FACTORY,
				googleClientSecrets, SCOPES).setApprovalPrompt("auto").setAccessType("online")
	...
	* 발급된 authorization code는 Google cloud project 에 등록한 client app의 redirect_url 핸들러를 이용하여 받을 수 있음.
	...
	public String sendAuthorizationCodeRequest() {
		return authorizationCodeFlow.newAuthorizationUrl().setRedirectUri(details.getRedirectUris().get(0))
