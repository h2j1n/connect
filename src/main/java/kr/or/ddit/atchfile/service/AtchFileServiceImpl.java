...
import kr.or.ddit.atchfile.dao.AtchFileMapper;
...
public class AtchFileServiceImpl implements AtchFileService {
	...
	public void createAtchFile(AtchFileVO atchFile, File saveFolder){
		Optional.of(atchFile)
				...
				.ifPresent(
					fds->
						fds.forEach(Failable.asConsumer(fd->fd.uploadFileSaveTo(saveFolder))
					)
				);
	...
	public void removeAtchFileDetail(int atchFileId, int fileSn, File saveFolder) {
		...
		Optional.ofNullable(target)
				.ifPresent(Failable.asConsumer(fd->deleteFileDetail(fd, saveFolder)));
	}
	...
	public AtchFileDetailVO retrieveAtchFileDetailByUUID(String streFileNm) {
		return mapper.selectAtchFileDetailByUUID(streFileNm);
	}
	...
	public int retrieveAtchFileMaxId() {
		return mapper.selectAtchFileMaxId();
	}
