   ...
   <div class="row invoice-preview">
       ...
       <main class="col-xl-9 col-md-8 col-12 mb-md-0 mb-6">
           <div class="card invoice-preview-card p-sm-12 p-6">
               ...
               <h3 class="text-center fw-bold pt-10 pb-5" id="title">
                       ...
                       <table class="table">
                           ...
                           <tr>
                               <td class="text-center">
	                               	<c:choose>
										<c:when test="${dcmDetail.pmtDcmRecStano eq 'DB01110'}"> 대기 </c:when>
										<c:when test="${dcmDetail.pmtDcmRecStano eq 'DB01120'}"> 예정 </c:when>
										<c:when test="${dcmDetail.pmtDcmRecStano eq 'DB01140'}"> 완료 </c:when>
									</c:choose>
                               </td>
                           </tr>
                       </table>
                       ...
                       <table class="table">
                           ...
                           <tr>
                               <c:forEach items="${dcmAppList}" var="dcmApp">
	                               <td class="text-center">
	                               	<c:choose>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06110'}"> 대기 </c:when>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06120'}"> 예정 </c:when>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06130'}"> 승인 </c:when>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06140'}"> 반려 </c:when>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06150'}"> 전결 </c:when>
									</c:choose>
	                               </td>
                               </c:forEach>
                           </tr>
                   ...
                   <table class="w-100 table">
                       <tbody class="border-top">
                           <tr class="border-bottom py-3">
                               <th class="px-4 w-120p">문서번호</th> <td>${dcmDetail.comdcmNum}</td>
                               <th class="px-4 w-120p">기 안 자</th> <td>${dcmDetail.pmtDcmEmpnm}</td>
                           </tr>
                           <tr class="border-bottom py-3">
               ...
               <c:if test="${not empty dcmDetail.pmtDcmAttno}">
             	<c:forEach items="${dcmDetail.atchFile.fileDetails }" var="fd" varStatus="stap">
             		<c:if test="${not empty fd.orignlFileNm}">
		             	<p class="mb-1">
		             	<!-- .fileDetails[0].streFileNm -->
	                  		<span class="me-2 h6">첨부파일 ${stap.index+1}:</span>
							<c:url value='/files/${dcmDetail.atchFile.fileDetails[0].streFileNm}' var="downUrl" />
							<a href="${downUrl }" download="${fd.orignlFileNm}">${fd.orignlFileNm}(${fd.fileFancysize })</a>
	           ...
	           <nav class="card-body">
	               ...
	               <c:forEach items="${dcmAppList}" var="dcmApp" varStatus="status">
		               <ul class="d-flex align-items-center px-0 mb-4 ">
		                   <li class="list-unstyled">
		                   		<c:choose>
		                   			<c:when test="${principal.realUser.empNo eq dcmApp.pmtDcmAppEmpId}">
		                   ...
		                   <li class="ms-auto list-unstyled ">
		                       		...
		                       		<c:choose>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06110'}">
											<span class="badge bg-label-info" >결재대기</span>
										</c:when>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06120'}">
											<span class="badge bg-label-success" >결재예정</span>
										</c:when>
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06130'}">
											<span class="badge bg-label-secondary" >결재승인</span>
										</c:when>
										...
										<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06150'}">
											<span class="badge bg-label-warning" >결재전결</span>
										</c:when>
									</c:choose>
		                   ...
		                   <li class="list-unstyled">
		                   		<c:choose>
		                   			<c:when test="${principal.realUser.empNo eq dcmApp.pmtDcmAppEmpId}">
		                   				<c:choose>
											<c:when test="${dcmApp.pmtDcmStaNo eq 'DB06110'}">
												<form:form modelAttribute="app" id="dynamicForm">
		                   							
		                   							<form:hidden path="pmtDcmAppNo"/>
		                   							<form:hidden path="pmtDcmAppEmpId"/>	
		                   							<form:hidden path="pmtDcmStaNo"/>		
                   									<form:textarea path="pmtDcmAppCmt" class="form-control" placeholder="코멘트 할 내용을 입력하세요."/>

							                    	<div class="row gap-3 mt-3 justify-content-center">
														<button type="button" class="btn btn-md btn-label-dark col-3" id="no-btn">반려</button>
														<!-- 전결가능한 직급 체크 -->
														<c:if test="${dcmDetail.pmtDcmPrePos >= dcmApp.pmtDcmAppEmpOneposNo and dcmAppList.size() != dcmApp.pmtDcmAppTrun}">														
															<button type="button" class="btn btn-md btn-label-dark col-3" id="pre-ok-btn">전결</button>
														</c:if>
														<button type="button" class="btn btn-md btn-dark col-3" id="ok-btn">승인</button>	
							                    	</div>
		                   							<form:hidden path="pmtDcmNo"/>
		                   							<form:hidden path="pmtDcmAppEmpNm"/>
		                   							<form:hidden path="pmtDcmAppEmpPos"/>
		                   							<form:hidden path="pmtDcmAppEmpOnepos"/>
		                   							<form:hidden path="pmtDcmAppTrun"/>
		                   							<form:hidden path="pmtDcmAppEmpDep"/>
		                   							<form:hidden path="pmtDcmAppEmpOneposNo"/>
		                   						</form:form>
											</c:when>
											<c:otherwise>
												<c:choose>
													<c:when test="${not empty dcmApp.pmtDcmAppCmt}">
	                   								...
	                   								<c:otherwise>
	                   									<span class="text-black-50">코멘트가 없습니다.</span>
	                   								</c:otherwise>
	                   							</c:choose>
											</c:otherwise>
										</c:choose>
	               ...
	               <ul class="d-flex align-items-center px-0">
	                   ...
	                   <li class="ms-auto list-unstyled ">
	                       ...
	                       <c:choose>
								<c:when test="${dcmDetail.pmtDcmRecStano eq 'DB01110'}"><span class="badge bg-label-info">처리대기</span></c:when>
								<c:when test="${dcmDetail.pmtDcmRecStano eq 'DB01120'}"><span class="badge bg-label-secondary">수신예정</span></c:when>
								<c:when test="${dcmDetail.pmtDcmRecStano eq 'DB01140'}"><span class="badge bg-label-success">처리완료 ${dcmDetail.pmtDcmRecDate}</span></c:when>
							</c:choose>
	                       
	                   </li>
	               </ul>
	               <ul class="border-bottom mb-2 px-0 text-center">
		               <c:if test="${principal.realUser.empNo eq dcmDetail.pmtDcmRecEmpno and dcmDetail.pmtDcmRecStano eq 'DB01110'}">
		               		<form:form modelAttribute="dcmDetail" id="recForm" method="post">
		               			<form:hidden path="pmtDcmNo"/>
		               			<button type="button" class="btn btn-md btn-dark mb-4" id="process-ok-btn">문서처리완료</button>
		               		</form:form>
		               </c:if>
	               </ul>
	               <ul class="px-0 mx-0 ">
	               		...
	               		<c:choose>
							<c:when test="${not empty refList}">
			               		<c:forEach items="${refList}" var="ref">
			               			<li class="list-unstyled">
			               				<c:choose>
		                   					<c:when test="${principal.realUser.empNo eq ref.pmtDcmRefEmpNo}">
		                   						
						                       <button class="btn btn-sm btn-label-danger text-nowrap d-inline-flex position-relative" data-bs-toggle="modal" data-bs-target="#empMethods" data-pmt-emp-no="${ref.pmtDcmRefEmpNo}">
						                       		${ref.pmtDcmRefEmpNm}
							...
							<c:otherwise>
								<span class="text-black-50">참조인원이 없습니다.</span>
							</c:otherwise>
						</c:choose>
	               </ul>
...
<!-- Page JS -->
<script defer src="${cPath}/resources/groupwareMember/assets/js/document-detail.js"></script>
<script defer src="${cPath}/resources/groupwareMember/assets/js/emp-modal.js"></script>
<script defer src="${cPath}/resources/groupwareMember/assets/js/document-sweetalert2.js"></script>