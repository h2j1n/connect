...
<a href="<c:url value='/prod/prodInsert.do'/>">신규 상품 등록</a>
<table class="table">
	<thead>
		<tr>
			<th>번호</th>
		...
		<c:choose>
			<c:when test="${not empty addressList}">
				<c:forEach items="${addressList}" var="address">
					<tr>
						<td>${address.rnum}</td>
						<td>${address.adrMarkyn}</td>
						<td><c:url value="/address/trashDetail.do"
								var="trashDetailUrl">
							...
							data-detail-url="${trashDetailUrl}"> ${address.adrNm} </a></td>
			...
			<c:otherwise>
				<tr>
					<td colspan="6">삭제된 외부연락처가 없습니다.</td>
				</tr>
			</c:otherwise>
		</c:choose>
	</tbody>
...
<script type="text/javascript" src="${cPath}/resources/js/app/hjadr/trashList.js"></script>
