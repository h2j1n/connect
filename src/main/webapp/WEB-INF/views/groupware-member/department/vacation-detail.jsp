									...
									<tr class="border-bottom py-3">
										...
										<td>
											<c:forEach items="${empList}" var="emp">
												<c:if test="${emp.empNo == empVacDetail.empNo}">
														${emp.empDepName }/ ${emp.empPosName }
												</c:if>
											</c:forEach>
							...
							<tr>
								...
								<td class="text-end ps-0 py-6 w-px-120 fw-medium text-heading">
									...
									<p class="fw-medium mb-2">
										${empVacDetail.empNm} <span class="stamp"> (인) <img
											src="${cPath}/resources/images/stamp/stamp_1.png" alt="신청도장">
										</span>
									</p>
								</td>
							</tr>
			...
			<div class="card">
				<div class="card-body">
					<div class="d-flex align-items-center mb-8">
						...
						<div class="ms-auto">
						<input type="hidden" id="appState" value="${empVacDetail.applicationList[0].appState}">
							<c:choose>
								<c:when
									test="${empVacDetail.applicationList[0].appState eq 'BB01120' }">
									<span class="badge bg-label-success" text-capitalized="">
										승인완료 </span>
								</c:when>
								<c:when
									test="${empVacDetail.applicationList[0].appState eq 'BB01110' }">
					...
					<form id="vacationForm" action="${cPath }/gw/department/approveApplication.do" method="post">
					<input type="hidden" name="appNo" value="${empVacDetail.applicationList[0].appNo}">
					<input type="hidden" name="empNo" value="${empVacDetail.empNo}">
					<input type="hidden" name="vaciNo" value="${empVacDetail.applicationList[0].vaciNo}">
					<input type="hidden" name="appStrdate" value="${empVacDetail.applicationList[0].appStrdate }">
					<input type="hidden" name="appEnddate" value="${empVacDetail.applicationList[0].appEnddate }">
					<input type="hidden" name="appTememp" value="${empVacDetail.applicationList[0].appTememp }">
					<input type="hidden" name="appDetail" value="${empVacDetail.applicationList[0].appDetail }">
					<input type="hidden" name="appDep" value="${empVacDetail.applicationList[0].appDep }">
					<input type="hidden" name="appDate" value="${empVacDetail.applicationList[0].appDate}">
					<input type="hidden" name="appRovemp" value="${empVacDetail.applicationList[0].appRovemp}">
					<input type="hidden" name="vaciType" value="${empVacDetail.applicationList[0].vaciType}">
						<div class="row gap-4 mt-5 justify-content-center">
						<c:choose>
						    <c:when test="${empVacDetail.applicationList[0].appState eq 'BB01110'}">
						        <button type="button" class="btn btn-md btn-secondary col-5" id="ok-btn">승인</button>
						        <button type="button" class="btn btn-md btn-label-dark col-5" id="no-btn">반려</button>
						    </c:when>
						    <c:otherwise>
						    </c:otherwise>
						</c:choose>
						...
						<div class="row gap-4 mt-5 justify-content-center">
					<button class="btn btn-primary w-90" id="print" target="_blank">
						<i class='bx bx-printer bx-sm me-2'></i> Print
					</button></div>
...
<script defer
	src="${cPath}/resources/js/app/jhjUa/vacation-detail.js"></script>
<script defer
