...
$(document).ready(function() {
    ...
    if (formRepeater.length) {
        formRepeater.on('submit', function (e) {
            ...
            // Convert inputs to key-value pairs
            inputs.forEach((input, index) => {
                if(index % 2 === 0) {
                    arr.push({ vaciNm: input.value });
                } else {
                    arr[arr.length - 1].vaciDay = input.value;
                }
            });
            ...
            $.ajax({
                url: formRepeater.attr('action'),
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(arr),
                success: function(response) {
                    location.href =`${contextPath}/gw/vacation/vacationList.do`;
                },
                error: function(xhr, status, error) {
                    alert('특별휴가 등록에 실패했습니다.');
                }
            });
        });
    }
...
$(document).ready(function() {
    ...
    if (formSpecial.length) {
        ...
        formSpecial.on('submit', function (e) {
        e.preventDefault();
		let inps = e.target.querySelectorAll("input");
		let arr = [];
			inps.forEach((inp)=>{
				arr.push(inp.value);
			})
