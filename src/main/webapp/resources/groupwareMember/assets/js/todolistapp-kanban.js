...
(async function () {
  ...
  const kanban = new jKanban({
    ...
    dragBoards: true,
    addItemButton: true,
    buttonContent: '+ 일정을 추가하세요',
    itemAddOptions: {
      enabled: true, // add a button to board for easy item creation
      content: '+ 일정을 추가하세요', // text or html content of the board button
    ...
    buttonClick: function (el, boardId) {
      ...
      addNew.innerHTML =
        '<div class="mb-4">' +
        '<textarea class="form-control add-new-item" rows="2" placeholder="일정을 입력해주세요" autofocus required></textarea>' +
        '</div>' +
        '<div class="mb-4">' +
