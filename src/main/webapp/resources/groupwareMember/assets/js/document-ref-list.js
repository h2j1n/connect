...
$(function () {
  ...
  var dt_adv_filter_table = $('.dt-advanced-search'),
    statusObj = {
        "DB01110": { title: '대기', class: 'bg-label-warning' },
        "DB01120": { title: '예정', class: 'bg-label-info' },
        "DB01130": { title: '처리', class: 'bg-label-secondary' },
        "DB01140": { title: '완료', class: 'bg-label-success' }
    },
    formObj = {
  ...
  if (dt_adv_filter_table.length) {
    var dt_adv_filter = dt_adv_filter_table.DataTable({
	  stateSave: true,
 	  stateSaveCallback: function (settings, data) {  localStorage.setItem( 'yj_2',JSON.stringify(data));  },
	  stateLoadCallback: function (settings) { return JSON.parse(localStorage.getItem('yj_2'));  },

      dom: "<'row'<'col-sm-12'tr>><'row'<'col-sm-12 col-md-6'i><'col-sm-12 col-md-6 dataTables_pager'p>>",
      ajax: {
			"url":contextPath+"/gw/document/refList",
			"dataSrc": ""
		},
      columns: [
        { data: '' },
      ...
      initComplete: function () {
		...
		const data = localStorage.getItem('yj_2');
		if (data) {
		    try {
		        // JSON 파싱
		        ...
		        let  category = parsedData.columns[3].search.search;
				category = category
				            .replace(/^\^/, '')     // 문자열 시작의 ^
				            .replace(/\\\//g, '/')  // \\/를 /
				            .replace(/\$$/, ''); 
		    ...
		    console.warn("로컬 스토리지에 'yj_2' 항목이 존재하지 않습니다.");
		}
