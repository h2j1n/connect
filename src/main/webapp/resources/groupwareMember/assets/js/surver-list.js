...
$(document).ready(function() {
	...
	const surveyType = surveyBody.dataset.surveyType;   
	let surveyTilte = ""; 
	...
	var table = $('#example').DataTable({
        ...
        ajax: {
			"url":contextPath+"/gw/survey/"+surveyType+"/myList",
			"dataSrc": ""
	  	},
        columns: [
            { data: 'svyAllYn' },
	    ...
	    initComplete: function() {
			...
			$('.titel').html(`<h5 class="fw-bold fs-4 mb-0">내가 ${surveyTilte} 설문</h5>`);
        });
	...
	function loadGrid(table) {
	    ...
	    data.each(function(rowData, index) {
			...
			let contFinish = ``;
			if(surveyType=="create"){
				contFinish =`<div class="d-flex justify-content-between align-items-center mb-2">
				...
				if(actialAns==0){
					contFinish ='<span class="badge bg-label-danger">설문에 답변을 완료하지 않았습니다.</span>'
				}else if(actialAns==1){ 
					contFinish ='<span class="badge bg-label-secondary">설문에 답변을 완료하였습니다.</span>'
				}
			}
